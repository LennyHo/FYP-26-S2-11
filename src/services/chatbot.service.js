...
// Answered deterministically from the DB record (see handler) so the sugar/calorie/grade values
// are NEVER generated by the model. This is the fix for the multi-language nutrition
// hallucination where the same question returned different fabricated sugar figures and an
// inverted Nutri-Grade in Malay vs the correct English answer.
...
// Health conditions are distinct from the transient symptoms above — they need a stronger
// "consult a professional" disclaimer and must never be diagnosed or treated. Routed to a
// dedicated handler BEFORE isRecommendationRequest so "I'm diabetic, what should I drink?" can't
// take the plain recommendation path (which carries no disclaimer). Fix for the diabetes-vs-
// pregnancy disclaimer gap, where the pregnancy question disclaimed but the diabetes one didn't.
...
function isHealthConditionRequest(message) {
    const msg = String(message || "").toLowerCase();
    return HEALTH_CONDITION_KEYWORDS.some((k) => msg.includes(k));
}
...
// Consecutive-unintelligible-message tracking for the human-handoff escape hatch (category E).
// In-memory per conversationId — intentionally lightweight (resets on server restart and is
// per-instance); the session model has no field for it and a durable counter isn't worth a schema
// change for a soft escalation nudge.
...
// size / ice / sugar / topping. Used so a bare customization correction ("actually make it less
// sugar") is applied to the pending drink instead of being re-read as a menu-wide "lowest sugar"
// ranking query.
...
function isRecommendationRequest(message) {
    ...
    // Every other named intent below also takes priority over drink recommendation. Without these
    // guards, this function's broad catch-alls ("give me", "show me", "what about X") swallow
    // requests meant for these other handlers before they're ever reached — e.g. "what about
    // driptea orchard opening hours?" or "give me my vouchers" would otherwise get misread as a
    // drink search, fail to match any drink, and return a generic "couldn't find a drink" reply
    // instead of the real answer. This function is checked early in the dispatch chain (before
    // voucher, store-info, tax, discount, and purchase-history), so it must explicitly defer to
    // them rather than assume it's the only thing that could match a "give me"/"show me" phrasing.
    if (
    ...
    return (
        ...
        // Exploratory / flavour-first responses (e.g. "maybe a matcha", "something fruity", "how about
        // taro") — only when paired with an actual drink/flavour word, so unrelated "what about X"
        // questions (a store name, opening hours, a policy...) aren't misread as a drink browse
        // request just because of the sentence opener. This is what let "what about driptea orchard
        // opening hours?" get swallowed here instead of reaching the store-info handler.
        (
...
function isTrackOrderRequest(message) {
    ...
    // A bare mention of "my order" inside a tax/discount/voucher question ("what about tax on my
    // order") is not actually asking to track the order — defer to those more specific handlers
    // instead of routing to "please log in to track your order status." Same class of bug as the
    // isRecommendationRequest guards above: a broad substring match shadowing a more specific intent.
...
// Detects a request to negotiate a custom discount / lower price ("give me a 90% discount",
// "can you make it cheaper", "any chance of a discount"). Without this, "90%" trips the
// customization-word check (it contains the substring "0%") and the message is misrouted to the
// add-to-cart handler, which then replies with an unrelated "which drink?" prompt. Handled with a
// clean decline that points to real vouchers instead.
...
// Detects tax / GST / service-charge questions. There is no stored tax policy anywhere in the
// system, so the model would otherwise invent one ("all prices include GST") — a policy claim the
// business could be held to. Handled with a redirect to the team instead of a fabricated answer.
...
const DRINK_NAME_ALIAS_PAIRS = [
    [["da hong bao", "da hong pao"], "Da Hong Bao Milk Tea"],
...
function isViewCartRequest(message) {
    ...
    // by the generic "my cart" / "the cart" match below — otherwise "clear my cart" or
    // "remove X from my cart" just re-displays the unchanged cart instead of mutating it.
    // Returning false here lets the message fall through to isClearCartRequest /
    // isCartUpdateRequest (or the add-to-cart ordering flow) which actually change the cart.
...
async function handleChatMessage({ message, conversationId, userId, isQuickPrompt = false }) {
    ...
    if (isHealthConditionRequest(intentMessage)) {
        ...
        const isSugarSensitive = SUGAR_SENSITIVE_KEYWORDS.some((k) => condMsg.includes(k));
    ...
    // Answered from the DB record so the numbers are never model-generated. Runs on intentMessage
    // (already translated to English), so it fixes the fabricated-in-Malay nutrition bug for every
    // language at once. Placed after the ranking/comparison check so "which has less sugar, A or B"
    // still goes to the comparison handler above.
    ...
    // Custom-discount negotiation ("give me a 90% discount", "make it cheaper") — decline cleanly
    // and point to real vouchers. Runs after the voucher handler (so "any discount?" still shows the
    // voucher card) and before add-to-cart (so it isn't misrouted there by the "0%"-in-"90%" quirk).
    ...
    // Tax / GST question — no stored tax policy exists, so redirect to the team rather than let the
    // model fabricate one.
    ...
    // midOrderModifier guard: "actually make it less sugar" mid-order is a change to the pending
    // (not-yet-carted) drink, so it must reach the order flow below, not the cart-edit handler
    // (which would fail with "couldn't find that item in your cart" since nothing is carted yet).
