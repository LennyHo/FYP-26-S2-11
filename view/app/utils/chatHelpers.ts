export const drinkData: Record<string, { id: string; category: string }> = {
...
export const DRINK_INFO: Record<string, { ingredients: string[]; diabeticAdvice: string; insulinImpact: string }> = {
  b001: {
...
export function convertDrinkNamesToLinks(text: string): string {
  let result = text;
...
export function applyGlossaryTooltips(html: string): string {
  let result = html;
  GLOSSARY_TERMS.forEach(term => {
    const regex = new RegExp(`\\b${term}\\b`, 'gi');
    const replacement = `<span title="${GLOSSARY[term] || ''}">${term}</span>`;
